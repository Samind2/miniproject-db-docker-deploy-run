    ...
    <div class="container-fluid px-4 px-lg-5">
        <nav class="py-2" aria-label="breadcrumb">
            <ol class="breadcrumb m-0">
                ...
                <li class="breadcrumb-item active text-truncate" aria-current="page">ผู้สมัคร</li>
            </ol>
        </nav>
    </div>
</div>
                        ...
                        <div class="row">
                            <div class="col-lg-3 mb-3">
                                ...
                                <select class="form-select" name="status" id="modifyForm_status">
                                    ...
                                    <?php else: ?>
                                        <option value="CONFIRM_PAID">ผ่านการตรวจสอบ</option>
                                    <?php endif ?>
    ...
    <div class="mb-4">
        <div class="card shadow border-0">
            <div class="card-body pb-0">
                <h3 class="fw-bolder text-center">ผู้สมัคร [<?= $course['title']; ?>] รอบ <?= $dateExtension::thai_range_format('d M Y', $courseBatch['start_classroom'], $courseBatch['end_classroom']); ?></h3>
                <div class="d-flex justify-content-between">
                    ...
                    <?php if($course['regis_fee'] > 0): ?>
                        <a href="<?= base_url() ?>/manage/courses/enroll/report/<?= $courseBatch['course_batch_id']; ?>" class="btn btn-light" target="_blank"><i class="fa-solid fa-file"></i> รายงาน</a>
                    <?php endif ?>
                </div>
            </div>
            <table class="table table-hover table-light w-100" id="enrollDT" style="min-width:1500px">
                <thead>
                    <th width="40">#</th>
            ...
            <script>
                ...
                $(d => {
                    let enrollDT = $('#enrollDT').DataTable({
                        ajax: {
                            url: '<?= base_url(); ?>/api/get/enroll/course/batch/dt/<?= $courseBatch['course_batch_id']; ?>/all',
                            method: 'POST',
                            error: e => {
                                Swal.fire({
                        ...
                        columnDefs: [{
                            ...
                            targets: 3,
                            createdCell: (td, cellData, rowData, row, col) => {
                                if (rowData[11]=='1') $(td).addClass('fw-bold');
                            }
                        }, {
                            targets: 6,
                            className: 'text-end'
                            ...
                            orderable: false,
                            createdCell: (td, cellData, rowData, row, col) => {
                                $(td).append('<button class="btn btn-light btn-sm '+(rowData[11]=='1' ? 'green-website-3-hex ' : '')+'handler-alert" data-id="'+rowData[10]+'" title="'+(rowData[11]=='1' ? 'ตั้งว่าอ่านแล้ว' : 'ตั้งยังไม่ได้อ่าน')+'">'+(rowData[11]=='1' ? '<i class="fa-solid fa-check">' : '<i class="fa-solid fa-bell">')+'</i></button>');
                    ...
                    $('select#statusEnrollment').on('change', e => enrollDT.ajax.url('<?= base_url(); ?>/api/get/enroll/course/batch/dt/<?= $courseBatch['course_batch_id']; ?>/'+$(e.currentTarget).val()).load());
